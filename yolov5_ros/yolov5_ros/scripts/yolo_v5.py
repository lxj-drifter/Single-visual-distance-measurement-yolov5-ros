...
class Yolo_Dect:
    ...
    real_hight_chair = 51.18      # 椅子高度
    # 自定义函数，单目测距
    def person_distance(self, h):
        foc = 350.0        # 镜头焦距
        real_hight_person = 66.9   # 行人高度
    ...
    def dectshow(self, org_img, boxs, height, width):
        ...
        for box in boxs:
            ...
            # bound_height = int(box[3]) - int(box[1])
            bound_height = int(box[3]) - int(box[1])
            if box[-1] == "person":
                d = self.person_distance(bound_height)
            elif box[-1] == "chair":
                d = self.chair_distance(bound_height)
            else:
                d = "none"

            cv2.putText(img, box[-1] + ' '+ str(d) + 'm',
